...
# 步驟 1A: 安裝資源檢查工具
echo "[安裝保護] 正在安裝資源檢查工具 (psutil, pyyaml, pytz, pydantic-settings)..."
"$VENV_PYTHON" -m pip install psutil pyyaml pytz pydantic-settings > /dev/null
if [ $? -ne 0 ]; then
    echo "❌ 錯誤：安裝資源檢查工具失敗。"
    exit 1
fi

# 步驟 1B: 執行 Python 資源檢查
echo "[安裝保護] 正在執行資源檢查..."
check_command="from src.phoenix_core.utils.resource_monitor import load_resource_settings, is_resource_sufficient; settings = load_resource_settings(); sufficient, msg = is_resource_sufficient(settings); print(msg); exit(0) if sufficient else exit(1)"
check_result=$("$VENV_PYTHON" -c "$check_command")
exit_code=$?

echo "[安裝保護] $check_result"

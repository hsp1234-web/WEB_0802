...
if str(project_root) != str(Path.cwd()):
    print(f"警告：此腳本應從專案根目錄 ({project_root}) 執行。")
    # 在某些情況下，我們可能仍希望它繼續執行，所以暫不退出
    # os.chdir(project_root) # 或者可以考慮切換目錄

def launch_process(command, name):
    """
    啟動一個子進程並返回其處理程序物件。
    將子進程的 stdout 和 stderr 導向到監督者的 stdout/stderr。
    """
    print(f"[Supervisor] 正在啟動 {name}...")
    # 使用 Popen 進行非阻塞啟動
...
def main():
    ...
    try:
        ...
        for name, command in components.items():
            processes[name] = launch_process(command, name)

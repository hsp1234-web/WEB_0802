...
def main():
    """
    執行並評測策略 A：原生 `venv` + `pip`。
    """
    strategy_name = "策略 A: 原生 venv + pip"
    print_header(f"開始測試: {strategy_name}")

    # --- 設定 ---
    project_root = Path(__file__).parent.parent.resolve()
    venv_dir = project_root / ".venv_A_temp"
    ...
    try:
        ...
        # --- 步驟 2: 安裝套件 ---
        # 在 Windows 上，路徑是 Scripts/python.exe
        if sys.platform == "win32":
            venv_python = venv_dir / "Scripts" / "python.exe"
        else:
            venv_python = venv_dir / "bin" / "python"

        print(f"\n步驟 2: 在虛擬環境中安裝 'pytz' (使用 --ignore-installed)...")
        subprocess.run(
            [str(venv_python), "-m", "pip", "install", "--ignore-installed", "pytz"],
            check=True,
            capture_output=True
        )
        print("✅ 套件 'pytz' 安裝成功。")
        ...
        print(f"❌ 執行期間發生錯誤！")
        # 使用 .decode(errors='ignore') 來避免潛在的編碼問題

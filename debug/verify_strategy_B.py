...
def main():
    ...
    執行並評測策略 B：原生 `uv`。
    """
    strategy_name = "策略 B: 原生 uv"
    print_header(f"開始測試: {strategy_name}")

    # --- 設定 ---
    project_root = Path(__file__).parent.parent.resolve()
    venv_dir = project_root / ".venv_B_temp"
    uv_executable = "uv"
    ...
    try:
        # --- 前置步驟: 檢查 uv 是否存在 ---
        print("步驟 0: 檢查 'uv' 是否在 PATH 中...")
        # Use a simple command that should succeed if uv is installed
        subprocess.run([uv_executable, "--version"], check=True, capture_output=True)
        print("✅ 'uv' 指令可執行。")
        ...
        # --- 步驟 1: 建立虛擬環境 ---
        print(f"\n步驟 1: 使用 '{uv_executable}' 建立虛擬環境...")
        subprocess.run(
            [uv_executable, "venv", str(venv_dir)],
            check=True,
            capture_output=True
        )
        ...
        print(f"\n步驟 2: 在虛擬環境中安裝 'pytz'...")
        subprocess.run(
            [uv_executable, "pip", "install", "pytz", "--python", str(venv_python)],
            check=True,
            capture_output=True
        )
        print("✅ 套件 'pytz' 安裝成功。")
        ...
        print("❌ 錯誤: 'uv' 指令找不到。請確保它已經被安裝並加入到系統的 PATH 環境變數中。")
        final_result = "錯誤 (uv not found)"

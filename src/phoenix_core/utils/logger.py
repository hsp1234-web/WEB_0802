...
class Logger:
    ...
    def archive_logs_to_file(self, shared_state: dict) -> Optional[str]:
        """
        將資料庫中的所有日誌歸檔到一個文字檔案中，並在檔案頂部加入執行摘要。

        Args:
            shared_state (dict): 包含最終狀態的共享字典。
            ...
            try:
                ...
                with open(archive_filepath, 'w', encoding='utf-8') as f:
                    ...
                    if shared_state.get('error'):
                        final_status = f"❌ 失敗 ({shared_state.get('error')})"
                    elif not shared_state.get('worker_finished', False):
                         final_status = "⚠️ 未知 (Worker 未正常結束)"
                    f.write(f"總體結果: {final_status}\n")

                    # 時間統計
                    if all_logs:
                        start_time = datetime.fromisoformat(all_logs[0][0])
                        end_time = datetime.fromisoformat(all_logs[-1][0])
                        duration = end_time - start_time
                        f.write(f"開始時間: {start_time.isoformat()}\n")
                        f.write(f"結束時間: {end_time.isoformat()}\n")
                        f.write(f"總 耗 時: {duration}\n")

                    f.write(f"日誌總數: {len(all_logs)}\n")

...
def main():
    ...
    run_command(["uv", "venv", str(venv_path)], "使用 uv 建立虛擬環境")
    ...
    print("📊 探測任務完成: 性能報告")
    print("="*50)
    print(f"虛擬環境建立 ({'uv venv'}): {venv_setup_time:.2f} 秒")
    print(f"依賴安裝 ({'uv pip install'}): {install_deps_time:.2f} 秒")
    print("-" * 50)
    print(f"總環境準備耗時: {total_time:.2f} 秒")
    print("\n✅ {LOG_PREFIX} 探測成功！這證明了 'uv' 工具鏈在此環境中的高效與穩定性。")
    print(f"{LOG_PREFIX} 下一步可以安全地將此邏輯整合到主啟動腳本中。")
